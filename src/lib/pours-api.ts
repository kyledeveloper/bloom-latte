...
import { displayPhotoSrc, pourPhotoUrl } from "@/lib/photo-store";
...
function toPour(row: PourRow): Pour {
  ...
  return {
    ...
    createdAt: created,
    photo: row.photo
      ? displayPhotoSrc(row.id, row.photo)
      : pourPhotoUrl(row.id),
    pattern,
...
export const listPours = createServerFn({ method: "GET" })
  ...
  .handler(async ({ context }) => {
    ...
    const rows = await sql<PourRow>`
      select id, created_at, pattern, rating, beans, milk, grind, notes
      from pours
...
export const updatePour = createServerFn({ method: "POST" })
  ...
  .handler(async ({ context, data }) => {
    ...
    const replacePhoto = data.photo.startsWith("data:image/");
    if (replacePhoto) {
      await sql`
        update pours
        set
