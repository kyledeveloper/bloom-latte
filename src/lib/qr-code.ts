/* Project Nayuki QR Code Generator - converted to ESM */
type bit = number;
type byte = number;
type int = number;
...
export const DEFAULT_WEBSITE_URL = "https://bloom-latte.vercel.app";

export function getWebsiteShareUrl(): string {
  if (typeof window !== "undefined") {
    ...
    const publicHost = (import.meta as any).env?.VITE_PUBLIC_HOSTNAME;
    if (publicHost && typeof publicHost === "string" && publicHost.trim()) {
...
export function drawQrBadge(
  ...
  const {
    ...
    y,
    size,
    foregroundColor = "#1a1510",
    backgroundColor = "#ffffff",
    cardBackgroundColor = "#faf6f0",
    borderColor = "#e8ddd0",
    borderRadius = 16,
    label,
    labelColor = "#7a6d62",
  ...
  const cardHeight = size + labelHeight;
  ...
  ctx.moveTo(x + r, y);
  ctx.arcTo(x + size, y, x + size, y + cardHeight, r);
  ctx.arcTo(x + size, y + cardHeight, x, y + cardHeight, r);
  ctx.arcTo(x, y + cardHeight, x, y, r);
  ...
  // Draw inner white box for optimal QR code scanning contrast & quiet zone
  const innerPad = 8;
  ...
  ctx.arcTo(innerX + innerSize, innerY, innerX + innerSize, innerY + innerSize, innerR);
  ctx.arcTo(innerX + innerSize, innerY + innerSize, innerX, innerY + innerSize, innerR);
  ctx.arcTo(innerX, innerY + innerSize, innerX, innerY, innerR);
  ctx.arcTo(innerX, innerY, innerX + innerSize, innerY, innerR);
  ctx.closePath();

  ctx.fillStyle = backgroundColor;
  ctx.fill();

  // Standard 4-module quiet zone on each side
  const quietZone = 4;
  const totalModules = matrix.size + quietZone * 2;
  const cellSize = innerSize / totalModules;

  ctx.fillStyle = foregroundColor;
  for (let row = 0; row < matrix.size; row++) {
    for (let col = 0; col < matrix.size; col++) {
      if (matrix.modules[row][col]) {
        const mx = Math.round(innerX + (col + quietZone) * cellSize);
        const my = Math.round(innerY + (row + quietZone) * cellSize);
        const mw = Math.round(innerX + (col + quietZone + 1) * cellSize) - mx;
        const mh = Math.round(innerY + (row + quietZone + 1) * cellSize) - my;
        ctx.fillRect(mx, my, mw, mh);
      }
    }
  }

  // Label caption below the QR box
  if (label) {
